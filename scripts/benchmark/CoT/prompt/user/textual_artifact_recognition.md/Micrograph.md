...
- All textual elements (e.g., annotations and legends) are semantically meaningful and free from garbled, misspelled, or pseudo-text characters; for example, module names in a pipeline, place names, or relationship labels are valid, interpretable, and contextually appropriate.
- Text elements are functionally grounded, providing explicit references to specific components or positions in the figure, with correct numerical ordering and scale annotation, accurately placed labels, and logically consistent tick values.
