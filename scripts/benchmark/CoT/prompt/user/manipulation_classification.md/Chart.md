...
- In charts such as bar charts, line charts, heatmaps, or similar visualizations, chart elements (e.g., bars, lines, plotted data), textual components, legends, and annotations are considered foreground objects.
...
Alter should only be selected when the red-highlighted region corresponds to the same pre-existing visual element (e.g., the same bar, the same curve, the same error bar), and the change is limited to its appearance—such as color, fill pattern, thickness, shape, or texture, **without introducing any new independent graphical elements**.

- In chart-based images, treat each bar, curve segment, shaded region, marker, or heatmap cell as an independent graphical primitive. If a new primitive appears in a region that previously functioned as background or empty plot space, classify the manipulation as Insert, even if the primitive resembles existing chart elements.
